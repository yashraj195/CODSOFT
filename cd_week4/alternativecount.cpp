...
int main() {
    ...
    // Ask user for input in the form of a production rule (e.g., A -> i / jo / wep)
    cout << "Enter a production rule (e.g., A -> i / jo / wep): ";
    getline(cin, input);

    // Validate the entire production rule
    if (isValidProductionRule(input)) {
        // Extract RHS to count the number of alternatives
        size_t arrowPos = input.find("->");
        string rhs = input.substr(arrowPos + 2); // Get RHS (after "->")
        
        int numAlternatives = countAlternatives(rhs);

        cout << "Valid production rule!" << endl;
        cout << "Number of alternatives in RHS: " << numAlternatives << endl;
    } else {
